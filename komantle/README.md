...
`web_data/komantle-data/` 에 `fp.bin`, `meta.json`, `n/0.json` … `n/4649.json`, `cand.bin`, `vec/` 가 생깁니다. 약 135MB이며 git에 올리지 않습니다.

- `export_vectors.py`는 어휘 59,118개의 단어 벡터(float16)와 후보 4,650개의 벡터를 내보냅니다. 단어 단서로 정답을 찾을 때 브라우저가 유사도를 직접 계산하는 데 씁니다.
...
- 데이터를 다시 만들었으면(`export_web_data.py`, `export_vectors.py` 재실행) `npm run deploy` 를 다시 실행하면 됩니다. 데이터 파일은 하루(`max-age=86400`) 캐시되므로 방문자에게는 최대 하루 뒤에 반영됩니다.

## 4. 사용법

1. 꼬맨틀 첫 화면 상단 문장을 복사해 붙여넣거나, 1위·10위(필수)·1,000위(선택) 숫자를 입력하고 **찾기**.
   - "세 숫자가 정확히 일치하지 않습니다"가 뜨면 **단어 단서**를 펼쳐, 꼬맨틀에 이미 친 단어와 그때 나온 유사도를 넣고 다시 찾기. 어떤 단어든 되고(유사도가 낮아도 됨), 하나로 부족하면 두 개를 넣습니다.
